...
const patch=()=>{
 ...
 if(list)list.querySelectorAll('button[data-admin]').forEach(b=>{const d=/disabled/i.test(b.closest('.row')?.textContent||'');b.textContent=d?'Enable':'Disable';b.classList.toggle('danger',!d)});
 const body=document.getElementById('ceStudentRows');
 if(body)body.querySelectorAll('button[data-toggle-student]').forEach(b=>{const d=/disabled/i.test(b.closest('tr')?.textContent||'');b.textContent=d?'Enable':'Disable';b.classList.toggle('danger',!d)});
