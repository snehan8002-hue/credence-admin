...
function modal(title,fields,onSave){$('#csWorkModal')?.remove();const box=document.createElement('div');box.id='csWorkModal';box.className='cs-modal';box.innerHTML=`<div class="cs-modalbox"><div class="cs-modalhead"><h3>${title}</h3><button class="cs-x" id="csClose">×</button></div><div class="cs-formgrid">${fields.map(f=>`<div class="${f.full?'full':''}"><label>${f.label}</label>${f.type==='textarea'?`<textarea id="${f.id}" placeholder="${f.ph||''}"></textarea>`:`<input id="${f.id}" type="${f.type||'text'}" placeholder="${f.ph||''}" value="${esc(f.value||'')}">`}</div>`).join('')}</div><div class="cs-modalactions"><button class="cs-btn alt" id="csCancel">Cancel</button><button class="cs-btn" id="csSave">Save & publish</button></div></div>`;document.body.appendChild(box);$('#csClose').onclick=()=>box.remove();$('#csCancel').onclick=()=>box.remove();$('#csSave').onclick=async()=>{try{const data={};fields.forEach(f=>data[f.key||f.id]=$('#'+f.id)?.value?.trim()||'');await onSave(data);box.remove()}catch(e){toast(e?.message||'Could not save this item.')}}}
...
window.__csAction=async id=>{if(['video','announce','class'].includes(id))return window.__csCreate(id);const b=document.querySelector(`[data-nav="${id}"]`);if(b)b.click();else toast('Workspace action ready.');};
