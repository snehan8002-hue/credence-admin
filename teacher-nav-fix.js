...
function isVisible(el){if(!el)return false;const s=getComputedStyle(el);return s.display!=='none'&&s.visibility!=='hidden'&&el.getBoundingClientRect().width>0&&el.getBoundingClientRect().height>0}
function findTeacherModal(){
 const mods=[...document.querySelectorAll('.modal')];
 return mods.find(m=>isVisible(m)&&/add teacher|create teacher|teacher name/i.test(clean(m.textContent)))
   ||mods.find(m=>/add teacher|create teacher|teacher name/i.test(clean(m.textContent))&&m.style.display!=='none');
}
...
function ensureTeacherForm(){
 ...
 [...modal.querySelectorAll('.field')].forEach(f=>{if(/^classes?$/i.test(clean(f.querySelector('label')?.textContent))){f.remove();}});
...
function patchTeacherRows(){const body=document.getElementById('ceTeacherRows');if(!body)return;body.querySelectorAll('tr').forEach(r=>{const rm=r.querySelector('[data-remove-teacher]');if(!rm)return;const id=rm.dataset.removeTeacher;if(!id)return;let b=r.querySelector('[data-teacher-status-action]');const disabled=!!r.querySelector('.badge.cancel');if(!b){b=document.createElement('button');b.type='button';b.dataset.teacherStatusAction='1';b.className='btn '+(disabled?'alt':'danger');rm.parentElement.insertBefore(b,rm);}b.textContent=disabled?'Enable':'Disable';b.className='btn '+(disabled?'alt':'danger');b.onclick=e=>{e.preventDefault();e.stopPropagation();toggleTeacher(id,disabled)};});}
